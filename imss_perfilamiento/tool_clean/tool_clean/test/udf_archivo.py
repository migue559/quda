...
from test.udf_ayu_archivo import udf_ayu_agr_obj_archivos, udf_ayu_agr_reglas, udf_ayu_val_ori_datos, udf_ayu_val_exi_archivo
...
def udf_delimitador(menu, resultado):
    ...
    while True:
        ...
        if str(v_opcion_menu) in tupla_opciones:
            ...
            print(Style.BRIGHT + Fore.RED + '\n¡Aviso! ', Style.BRIGHT + Fore.WHITE + 'La opción',
                  Style.BRIGHT + Fore.WHITE + str(v_opcion_menu),
                  Style.BRIGHT + Fore.WHITE + ' no es válida, intente de nuevo.')
    ...
    if resultado != -1:  # tupla_archivo = udf_pide_archivo_s(v_respuesta) = (v_archivo, v_opcion_header, v_delimitador)
        ...
        for idx, archivo in enumerate(resultado.archivos):
            ...
            if resultado.ban_exi_archivo:
                ...
                resultado.par_pyspark = '{ruta}pars_{var}_{var_1}.json'.format(ruta=resultado.ruta_par_pyspark, var=idx,
                                                                               var_1=archivo.nombre.replace('.csv', ''))
                ...
                print(resultado.msj_trt_archivo_no,
                      Style.BRIGHT + str(resultado.ruta_src) + str(archivo.nombre))  # , resultado.reglas)}
