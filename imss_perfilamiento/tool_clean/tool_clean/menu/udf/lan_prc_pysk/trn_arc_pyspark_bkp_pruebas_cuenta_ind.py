...
conf = pyspark.SparkConf().setAppName("trn_arc_pyspark_bkp_pruebas_cuenta_ind").setMaster("local[4]")
...
header_list=[]
for idx,elem in enumerate(header.split("^")):
  header_list.append("column_"+str(idx+1))
temp_var = file_txt.map(lambda k: k.split("^"))
#here's where the changes take place
#this creates a dataframe using whatever pyspark feels like using (I think string is the default). 
#the header.split is providing the names of the columns
...
sp_df=temp_var.toDF(header_list)
...
#sp_df.repartition(1).write.csv(path='tmp/'+src, mode="append", header="false",sep=par['separador'])
sp_df.repartition(1).write.csv(path='tmp/'+src, mode="overwrite", header="false",sep=par['separador'])
#sp_df.write.format("com.databricks.spark.csv").save("tmp/"+src, header="false",,sep=par['separador'])
...
hdfs.put('tmp/'+src+'/'+fileName, '/dev/la/lt_aficobranza/H_SINDO_SSCI_CUENTA_INDIVIDUAL/'+fileName)
